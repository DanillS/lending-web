...
## Подсказки адреса не выпадают

1. `/health/ready` → `"dadata": false` — пустой `DADATA_API_KEY`. Ключ в [dadata.ru/profile](https://dadata.ru/profile/#info), вписать в `.env`, пересоздать `api`.
2. Ключ есть, список пустой — таймаут или 403 (не подтверждена почта / дневной лимит). Заявку с руками вписанным адресом это не блокирует.
3. Телефон на checkout всё равно нормализуется в `+7…` без секрета DaData.
